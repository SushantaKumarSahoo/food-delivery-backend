...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('QuickBite Subscription Service')
    .setDescription('Subscription plans and billing API')
    .setVersion('1.0')
    .addBearerAuth()
    .build();
  ...
  const port = parseInt(process.env.PORT_SUBSCRIPTION_SERVICE || '3014', 10);
